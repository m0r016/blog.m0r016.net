...
+   ReadMe: /readme
{% endcodeblock %}
AboutやReadMeは初めから生成されておらず、ブラウザからアクセスすると404エラーが出る。そのため `hexo new page "about"` と打つと`source/_posts`下に生成されるのではなく`source/`下に生成され、参照できるようになる。
